...
class CentrifugoClientManager private constructor(context: Context) {
    ...
    private val _lastDataSent = MutableStateFlow<DroneData?>(null)
    val lastDataSent: StateFlow<DroneData?> = _lastDataSent
    ...
    private var  currentDroneData: DroneData? = null

    // API service for token generation
    private val apiService: CentrifugoApiService by lazy {
        val baseUrl = settingsManager.getBaseUrl().let { url ->
        ...
        Retrofit.Builder()
            ...
            .client(authManager.getHttpClient())
            .build()
            .create(CentrifugoApiService::class.java)
    }
    ...
    /**
     * Update drone data to be sent to Centrifugo
     */
    fun updateDroneData(droneData: DroneData) {
        currentDroneData = droneData
    }
...
//                            Log.d(TAG, "Successfully published to channel $channel")
        ...
        try {
            ...
            val eventListener = object : EventListener() {
                override fun onConnected(
                    ...
                    if (droneId == null) {
                        Log.e(TAG, "Invalid token: unable to extract drone ID")
                        _connectionState.value = CentrifugoConnectionState.ERROR
                        return
                    }

                    val channel = "mobile-data:$droneId"
    ...
    private fun startDataTransmission(channel: String) {
        dataTransmissionJob = scope.launch {
            try {
                while (isActive) {
                    currentDroneData?.let { data ->
                        // Publish the latest drone data at regular intervals
